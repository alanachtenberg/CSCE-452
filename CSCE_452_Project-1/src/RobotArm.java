...
public class RobotArm extends Canvas{
	...
	private final Point[] relativeStarts= new Point[NUMLINKS];//begining of links
	private final float[] thetas= new float[NUMLINKS];
	private Point origin;//origin point, point of arm base in absolute coordinates

	private DHParams params;

	private MouseListener mListener;//pointer to our mouse listener
	private MouseMotionListener mMotionListener;//pointer to our mouse motion listener
	...
	public void setTheta(float t1, float t2, float t3){
		...
		thetas[0]=t1;//set thetas in local variable
		thetas[1]=t2;//will allow us to request what the current thetas are
		thetas[2]=t3;//fore incrementing and decrementing
